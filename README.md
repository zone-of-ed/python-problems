# python-problems

## Print
1. Write a program that print 'Hello World' as output.   
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Hello World
    > </pre> 
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/01.py" target="_blank">solution</a>
    [solution2](./print/01.py)

2. Write a program that print the number 12345 as output.
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 12345
    > </pre>  
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/02.py" target="_blank">solution</a>

3. Write a program that prints the following output:  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > a
    > ab
    > abc
    > </pre>  
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/03.py" target="_blank">solution</a>

4. Write a program that prints the following output:  
   > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 1
    > 12
    > 123
    > </pre>  
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/04.py" target="_blank">solution</a>

5. Write a program that prints the following output:  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > *
    > **
    > ***
    > </pre>   
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/05.py" target="_blank">solution</a>

6. Write a program that prints the following output:  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > ***
    > ***
    > ***
    > </pre>   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/06.py" target="_blank">solution</a>

7. Write a program that prints the following output:  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    >   *
    >  ***
    > *****
    > </pre>
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/07.py" target="_blank">solution</a>
 
8. Write a program that prints the following output:  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > *****
    >  ***
    >   *
    > </pre>
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/08.py" target="_blank">solution</a>

9. Write a program that print your date of birth.
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 01-jan-2030
    > </pre>
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/09.py" target="_blank">solution</a>

10. Write a program that print your name, age, and country on separate lines.
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > My name is zoneofed
    > Iam 10 years old
    > Iam from India
    > </pre>
   <a href="https://github.com/zone-of-ed/python-problems/blob/main/print/10.py" target="_blank">solution</a>

## Variables

1. Write a program that create a variable named user_name, assign your name as a string to it, and print the variable.  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > zone of ed
    > </pre>
   
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Follow Python variable rules while creating variables.
    > </pre>
   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/01.py" target="_blank">solution</a>

2. Write a program that create a variable named age, assign your age as a number to it, and print the variable.  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 20
    > </pre>
   
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Follow Python variable rules while creating variables.
    > </pre>
   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/02.py" target="_blank">solution</a>

3. Write a program that create a variable named gravity, assign 9.81 as a value to it, and print the variable.  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 9.81
    > </pre>
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/03.py" target="_blank">solution</a>

4. Write a program that create a variable named birth_year, assign your birth year as a number to it, and print the variable.  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 3000
    > </pre>
   
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Follow Python variable rules while creating variables.
    > </pre> 
   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/04.py" target="_blank">solution</a>

5. Write a program that creates variables for username, age, email ID, phone number, and date of birth, and assigns your details to them.  
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > username: zone of ed
    > age: 20
    > email id: zoneofed@gmail.com
    > phone number: 9876543210
    > date of birth: 22/01/2050
    > </pre>
   
     > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Follow Python variable rules while creating variables.
    > </pre> 
   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/05.py" target="_blank">solution</a>

6. Write a program to declare multiple variables on the same line.  
   > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 1
    > 2
    > 3
    > </pre> 

    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > value of a is 1
    > value of b is 2
    > value of c is 3
    > </pre>
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/06.py" target="_blank">solution</a>

7. Write a program that swap the values of two variables.  
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > a = 1
    > b = 2
    > </pre> 

    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > a: 2
    > b: 1
    > </pre>
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/07.py" target="_blank">solution</a>

8. Write a program to swap two numbers without using third variable.  
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > a = 1
    > b = 2
    > </pre> 

    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > a: 2
    > b: 1
    > </pre>
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/variables/08.py" target="_blank">solution</a>

## Input

1. Write a program that asks the username as input and print the username.   
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > zone of ed
    > </pre> 
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Your username is: zone of ed
    > </pre> 
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/input/01.py" target="_blank">solution</a>

2. Write a program that asks the user age as input and print the age.   
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > 20
    > </pre> 
   
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Your age is: 20
    > </pre> 

    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > The prompt should be: Please enter your username: 
    > </pre> 
   
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/input/02.py" target="_blank">solution</a>

3. Write a program that asks the user hobbies as input and print the hobbies.   
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    >  Watching TV, Cricket, Sleeping
    > </pre> 
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Your hobbies are Watching TV, Cricket, Sleeping
    > </pre> 
    
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > The prompt should be: Please enter your hobbies:
    > </pre> 

    <a href="https://github.com/zone-of-ed/python-problems/blob/main/input/03.py" target="_blank">solution</a>

4. Write a program that asks the user first name and last name as inputs and prints both the first name and last name on one line.   
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Surya
    > Chandra
    > </pre> 
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Your full name is Surya Chandra
    > </pre> 
    
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Please use a prompt for each input: one for the first name and one for the last name.
    > </pre> 

    <a href="https://github.com/zone-of-ed/python-problems/blob/main/input/04.py" target="_blank">solution</a>
        
5. Write a program that asks the user for their favorite movie and its director.   
    > **Input:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Batman Begins
    > Christopher Nolan
    > </pre> 
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > Your favorite movie is Batman Begins, directed by Christopher Nolan
    > </pre> 
    
    > ***Hint:***  
    > <pre style="background-color: #303135; padding: 5px;">
    > Please use a prompt for each input: one for the movie and one for the director.
    > </pre> 

    <a href="https://github.com/zone-of-ed/python-problems/blob/main/input/05.py" target="_blank">solution</a>

## Datatypes

1. Write a program to declare a positive integer.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/01.py" target="_blank">solution</a>

2. Write a program to declare a negative integer.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/02.py" target="_blank">solution</a>

3. Write a program to declare a positive integer using the int class.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/03.py" target="_blank">solution</a>

4. Write a program to declare a negative integer using the int class.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/04.py" target="_blank">solution</a>

5. Write a program to declare multiple integer values on the same line.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/05.py" target="_blank">solution</a>

6. Write a program to declare the large integer 7,654,321 using underscores as separators.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/06.py" target="_blank">solution</a>

7. Write a program to declare a positive float.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/07.py" target="_blank">solution</a>

8. Write a program to declare a negative float.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/08.py" target="_blank">solution</a>

9. Write a program to declare a positive float using the float class.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/09.py" target="_blank">solution</a>

10. Write a program to declare a negative float using the float class.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/10.py" target="_blank">solution</a>

11. Write a program to declare multiple float values on the same line.   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/11.py" target="_blank">solution</a>

12. Write a program to declare the PI value   
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/datatypes/12.py" target="_blank">solution</a>


## TypeCasting

## Operators - Arithmetic

1. Write a program for sum of  34 and 45.   
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > The sum of 34 and 45 is 79
    > </pre> 

    <a href="https://github.com/zone-of-ed/python-problems/blob/main/operators_arithmetic/01.py" target="_blank">solution</a>

2. Write a program for product of  64 and 4.   
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > The product of 64 and 4 is 256
    > </pre> 
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/operators_arithmetic/02.py" target="_blank">solution</a>

3. Write a program to find the quotient of 6 divided by 2.   
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > The quotient of 6 divided by 2 is 3.0
    > </pre> 
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/operators_arithmetic/03.py" target="_blank">solution</a>

4. Write a program to find the reminder of 6 divided by 2.   
    
    > **Output:**  
    > <pre style="background-color: #303135; padding: 5px;">
    > The remainder of 6 divided by 2 is 0
    > </pre> 
    
    <a href="https://github.com/zone-of-ed/python-problems/blob/main/operators_arithmetic/02.py" target="_blank">solution</a>


## Operators - Assignment

## Operators - Comparison

## Operators - Logical

## Operators - Identity

## Operators - Membership

## Conditions

## Range

## Escaping Character

## For Loop

## While Loop

## Break and Continue

## String 

## List 

## Dictionary 

## Tuple 

## Sets 

## Function

## Recursive Function













